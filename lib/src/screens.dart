...
class LibraryScreen extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) { final s = context.watch<AppState>(); final fav = s.episodes.where((e) => s.favorites.contains(e.id)).toList(); final recent = s.history.map(s.byId).whereType<Episode>().toList();
    return DefaultTabController(length: 3, child: Column(children: [const Material(child: TabBar(tabs: [Tab(text:'Favoritos'),Tab(text:'Listas'),Tab(text:'Historial')])), Expanded(child: TabBarView(children: [EpisodeList(items: fav, empty: 'Aún no hay favoritos.'),
      ListView(padding: const EdgeInsets.all(16), children: [FilledButton.icon(onPressed: () => _createPlaylist(context, s), icon: const Icon(Icons.add), label: const Text('Nueva lista')), ...s.playlists.map((p) => ExpansionTile(title: Text(p.name), subtitle: Text('${p.items.length} elementos'), trailing: IconButton(icon: const Icon(Icons.delete_outline), onPressed: () => s.deletePlaylist(p.id)), children: p.items.map((item) { final e = s.byId(item.episodeId); if (e == null) return const SizedBox.shrink(); return ListTile(title: Text(item.label.isEmpty ? e.title : item.label), subtitle: Text(item.seconds > 0 ? '${e.title} · ${clock(item.seconds)}' : e.title), leading: const Icon(Icons.play_circle_outline), onTap: () => context.read<AudioController>().play(e, atSeconds: item.seconds)); }).toList()))]), EpisodeList(items: recent, empty: 'El historial aparecerá al reproducir episodios.')]))]));
  }
}
...
void _playlistSheet(BuildContext context, AppState s, Episode e) { showModalBottomSheet(context:context,builder:(x)=>SafeArea(child:Column(mainAxisSize:MainAxisSize.min,children:[const ListTile(title:Text('Añadir o quitar de una lista')),if(s.playlists.isEmpty)const Padding(padding:EdgeInsets.all(20),child:Text('Crea primero una lista en Biblioteca.')),...s.playlists.map((p)=>CheckboxListTile(title:Text(p.name),value:p.items.any((i) => i.episodeId == e.id && i.seconds == 0),onChanged:(_)=>s.togglePlaylist(p.id,e.id)))]))); }
