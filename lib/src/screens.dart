...
class EpisodesScreen extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return RefreshIndicator(onRefresh: () => state.refresh(manual: true), child: CustomScrollView(slivers: [
      SliverAppBar.large(title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Coffee Break'), Text('${state.episodes.length} episodios${state.fromCache ? ' · sin conexión' : ''}', style: Theme.of(context).textTheme.bodySmall)])),
      SliverPadding(padding: const EdgeInsets.fromLTRB(12, 0, 12, 20), sliver: SliverList.builder(itemCount: state.episodes.length, itemBuilder: (_, i) => EpisodeTile(episode: state.episodes[i]))),
    ]));
  }
}
...
class _SearchScreenState extends State<SearchScreen> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Padding(padding: const EdgeInsets.symmetric(horizontal: 16), child: Align(alignment: Alignment.centerLeft, child: Text('${results.length} resultados'))),
...
class LibraryScreen extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) { final s = context.watch<AppState>(); final fav = s.episodes.where((e) => s.favorites.contains(e.id)).toList(); final recent = s.history.map(s.byId).whereType<Episode>().toList();
    return DefaultTabController(length: 3, child: Column(children: [const Material(child: TabBar(tabs: [Tab(text:'Favoritos'),Tab(text:'Listas'),Tab(text:'Historial')])), Expanded(child: TabBarView(children: [EpisodeList(items: fav, empty: 'Aún no hay favoritos.'),
      ListView(padding: const EdgeInsets.all(16), children: [FilledButton.icon(onPressed: () => _createPlaylist(context, s), icon: const Icon(Icons.add), label: const Text('Nueva lista')), ...s.playlists.map((p) => ExpansionTile(title: Text(p.name), subtitle: Text('${p.episodeIds.length} episodios'), trailing: IconButton(icon: const Icon(Icons.delete_outline), onPressed: () => s.deletePlaylist(p.id)), children: p.episodeIds.map(s.byId).whereType<Episode>().map((e) => EpisodeTile(episode:e)).toList()))]), EpisodeList(items: recent, empty: 'El historial aparecerá al reproducir episodios.')]))]));
  }
}
...
class MiniPlayer extends StatelessWidget { const MiniPlayer({super.key}); @override Widget build(BuildContext context) { final a=context.watch<AudioController>(); if(a.episode==null)return const SizedBox.shrink(); final max=(a.duration.inMilliseconds>0?a.duration.inMilliseconds:a.episode!.durationSeconds*1000).toDouble(); return Material(color:const Color(0xFF24201C),child:Column(mainAxisSize:MainAxisSize.min,children:[Slider(value:a.position.inMilliseconds.clamp(0,max.toInt()).toDouble(),max:max<=0?1:max,onChanged:(v)=>a.seek(Duration(milliseconds:v.toInt()))),ListTile(dense:true,title:Text(a.episode!.title,maxLines:1,overflow:TextOverflow.ellipsis),subtitle:Text('${clock(a.position.inSeconds)} / ${clock((max/1000).round())}'),trailing:IconButton(icon:Icon(a.playing?Icons.pause_circle:Icons.play_circle),onPressed:a.toggle))])); } }
...
void _playlistSheet(BuildContext context, AppState s, Episode e) { showModalBottomSheet(context:context,builder:(x)=>SafeArea(child:Column(mainAxisSize:MainAxisSize.min,children:[const ListTile(title:Text('Añadir o quitar de una lista')),if(s.playlists.isEmpty)const Padding(padding:EdgeInsets.all(20),child:Text('Crea primero una lista en Biblioteca.')),...s.playlists.map((p)=>CheckboxListTile(title:Text(p.name),value:p.episodeIds.contains(e.id),onChanged:(_)=>s.togglePlaylist(p.id,e.id)))]))); }
